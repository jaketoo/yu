...
buttons_ques = [
    ...
    [
        InlineKeyboardButton("بـايـروجـرام بـوت", callback_data="pyrogram_bot"),
        InlineKeyboardButton("تـلـيـثون بـوت", callback_data="telethon_bot"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if not is_bot:
        t = "» ✔️الان ارسل رقمك مع رمز دولتك , مثال :+2123456789''"
    else:
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "[ارسل الكود زي اللي في الصوره](https://telegra.ph/file/da1af082c6b754959ab47.jpg)» 🔍من فضلك افحص حسابك بالتليجرام وتفقد الكود من حساب اشعارات التليجرام. إذا كان\n  هناك تحقق بخطوتين( المرور ) ، أرسل كلمة المرور هنا بعد ارسال كود الدخول بالتـرتيب الي ف الصوره.- اذا كانت كلمة المرور او الكود  هي\n 12345 يرجى ارسالها بالشكل التالي 1 2 3 4 5 مع وجود مسـافـات بين الارقام اذا احتجت مساعدة @Mvhmed.", filters=filters.text, timeout=600)
